...
- 一、 搭建Kerberos5服务
  ...
  - 1.1 主节点安装KDC服务及配置
    ...
    - 1.1.2 修改`/var/kerberos/krb5kdc/kdc.conf`配置文件
    ...
    - 1.1.4 修改`/var/kerberos/krb5kdc/kadm5.acl`配置文件，给数据库管理员添加ACL权限，\*代表全部权限
    ...
    - 1.1.6 创建管理员用户，保存好创建时设置的密码(如果忘记后期可以使用`cpw`命令更新)，并导出**keytab**
  ...
  - 1.2 从节点配置KDC Client服务

    ```Shell
    yum -y install krb5-libs krb5-workstation
    ```

    - 1.2.1 拷贝主节点的`krb5.conf`和`root.keytab`配置并测试
...
- 二、 创建Nifi服务证书
  ...
  - 2.2 拷贝生成好证书到主从节点服务器下`NIFI`安装目录中的**conf**文件夹，如下：
...
- 三、 配置Zookeeper服务(**主从节点都需要操作**)
  ...
  - 3.1 在NIFI安装目录下创建`state/zookeeper`目录和`myid`文件
    ...
    > **注意：** 从节点上创建的myid为2，如：`echo -n '1' > state/zookeeper/myid`

  - 3.2 修改`zookeeper.properties`配置文件
  ...
  - 3.3 修改`state-management.xml`配置，在**zk-provider**节点下添加连接字符串
  ...
  - 3.4 修改`nifi.properties`文件，把内置的`zookeeper`启动和`cluster`设置成`true`，如下：
...
- 四、 配置Nifi Admin初始化
  ...
  - 4.2 添加`authorizer`到`authorizers.xml`，打开`file-provider`节点注释并添加如下内容：
  ...
  - 4.3 修改`login-identity-providers.xml `文件，打开`kerberos-provider`节点注释：
